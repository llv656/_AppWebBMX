$(document).ready(function() {
    const $input = $('#responsable_compra');
    const $suggestionsContainer = $('#contenedor_responsable');
  
    $input.on('input', function() {
      const inputValue = $input.val().toLowerCase();
      // Filtrar sugerencias que coincidan con la entrada del usuario
      const matchingSuggestions = sugerenciasResponsable.filter(suggestion => suggestion.nombre.toLowerCase().includes(inputValue));
