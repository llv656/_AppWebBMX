...
$(document).ready(function () {
    ...
    var table = $('#tblConsulta').DataTable({
        ...
        columnDefs: [
            ...
            { "width": "15%", "targets": 2 },  // Ajusta el ancho de la third columna al 15%
            { "width": "20%", "targets": 3 },  // Ajusta el ancho de la fourth columna al 20%
            { "width": "5%", "targets": 4 },  // Ajusta el ancho de la fifth columna al 15%
    ...
    // Manejo de clic en el icono de Font Awesome
    $('#tblConsulta tbody').on('click', '.toggle-info', function () {
        var additionalRow = $(this).closest('tr').next('.additional-info');
        additionalRow.toggleClass('hidden');
    });
...
$(document).ready(function () {
    ...
    // Evento listeners para los botones de navegación en el tiempo
    $("#nextDay").on("click", avanzarDia);
    $("#prevDay").on("click", retrocederDia);

    // Actualizar la fecha cada segundo (1000 milisegundos)
    // setInterval(actualizarFecha, 1000);
});

// Vista Semanal ContenedorTbl verificar error
$(document).ready(function () {
    //Logica para cambiar la vista al cargar la página
    cambiarVista();

     // Agrega un evento onchange al elemento select
     $("#opcionesVista").on("change", function () {
        // Llamada a la función para cambiar la vista cuando cambia la opción
        cambiarVista();
    });

    function cambiarVista() {
        // Obtén el valor seleccionado del select
        var seleccion = $("#opcionesVista").val();

        // Oculta todos los contenedores
        $(".contenedorTbl").hide();

        // Muestra el contenedor correspondiente según la opción seleccionada
        if (seleccion === "diaria") {
            $("#contenedorTabla").show();
        } else if (seleccion === "semanal") {
            $("#contenedorTblVS").show();
        }
        // Puedes agregar más condiciones para otras vistas si es necesario
    }
})
...
//     // Obtener el estado del icono desde el almacenamiento local
//     var iconState = JSON.parse(localStorage.getItem('iconState')) || {
//         class: 'fa-sort-down',
//     };

//     // Aplicar el estado del icono al cargar la página
//     $('#ocultarFiltroFormAv').removeClass('fa-sort-down fa-sort-up').addClass(iconState.class);

//     // Ajustar la visibilidad del formulario según el estado del icono
//     if (iconState.class === 'fa-sort-down') {
//         $('.contenedorFiltrarBSAV').hide();
//     }

//     $('#ocultarFiltroFormAv').click(function () {
...
//         // Actualizar el estado del icono en el almacenamiento local
//         iconState.class = $(this).hasClass('fa-sort-up') ? 'fa-sort-up' : 'fa-sort-down';
